// SPACEFORCE — Three.js scene: inclined two-rail apparatus, scoring deck,
// chrome ball, predicted-release ring, star field.
...
export class Stage {
  ...
  buildLights() {
    ...
    const rim = new THREE.PointLight(0x3fd8d0, 1.4, 3);
  ...
  buildStars() {
    ...
    for (let i = 0; i < N; i++) {
      const R = 5 + 6 * Math.random();
      const th = Math.random() * Math.PI * 2;
      const ph = Math.acos(2 * Math.random() - 1);
      pos[3 * i] = R * Math.sin(ph) * Math.cos(th);
      pos[3 * i + 1] = Math.abs(R * Math.cos(ph)) - 1.2;
  ...
  buildRig() {
    // rail frame: x along rails (uphill), y normal to rail-center plane, z transverse
    ...
    rodGeo.rotateZ(-Math.PI / 2);       // align along +x
    rodGeo.translate(railLen / 2, 0, 0);
    this.rodP = new THREE.Group();
    this.rodM = new THREE.Group();
    this.rodP.position.z = +P.d0 / 2;
    this.rodM.position.z = -P.d0 / 2;
    for (const grp of [this.rodP, this.rodM]) {
      const mesh = new THREE.Mesh(rodGeo, chrome);
    ...
    // cups: rim torus + wall + dark base
    const rimMat = new THREE.MeshStandardMaterial({ color: AMBER, metalness: 0.85, roughness: 0.35, envMapIntensity: 0.9 });
    ...
    // predicted-release ring (fold-aware, Eq. 7.6)
  ...
  makeBallTexture() {
    ...
    g.fillStyle = 'rgba(255,180,84,0.5)';
  ...
  drawBoard() {
    ...
    const sx = W / P.boardLen;                    // meters → px (x measured from -0.05)
    ...
    g.textAlign = 'left';
    g.fillText('RISK-OPTIMAL PULSED CONTROL · TWO-RAIL BALL GAME · J. GOMILA 2026', 24, H - 20);
